...
bot.on('message', async (msg) => {
    ...
    if (menuButtons.some(btn => text.includes(btn)) && adminState[chatId]) {
        ...
        if (state.action === 'wait_2fa_secret') {
            ...
            if (!newKey) {
                bot.sendMessage(chatId, "❌ *Invalid key.*", { parse_mode: 'Markdown' });
                delete adminState[chatId];
                return;
            }
            try {
                ...
                bot.sendMessage(chatId, "✅ *API Key added successfully!*", { parse_mode: 'Markdown' });
            } catch (e) { bot.sendMessage(chatId, "⚠️ *Error saving key.*", { parse_mode: 'Markdown' }); }
            delete adminState[chatId];
            return;
        }
    }
        ...
        // --- User: Get Number Flow ---
            ...
            try {
                ...
                if (res.data.success) {
                    ...
                    userLastOrder.set(chatId, { numId: res.data.number_id, phone: res.data.number, plat, country, createdAt });
