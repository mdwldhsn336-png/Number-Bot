...
app.get('/', (req, res) => res.send('Premium Fire OTP Bot v30.0 (Withdraw Fix & Full Admin Control) is Running!'));
...
async function getAppConfig() {
    try {
        ...
        return { per_otp_rate: 5, min_withdraw: 50, pay_methods: ['Binance'], reward_system: true, top_reward_on: true, stexsms_on: true, voltxsms_on: true, force_start: false, global_feed_on: true, ref_otp_commission: 0.5, bonus_top1: 50, bonus_top2: 30, bonus_top3: 20 }; 
    }
}
                        ...
                        if (config.reward_system !== false) {
                            ...
                            if(uDoc) {
                                if(uDoc.last_active_date !== todayStr) {
                                    ...
                                    uDoc.last_active_date = todayStr;
                                }
...
// 🟢 Foolproof Midnight Reset for Top Bonus
setInterval(async () => {
    const now = new Date();
    const bdTimeMs = now.getTime() + (now.getTimezoneOffset() * 60000) + (6 * 3600000);
    const bdTime = new Date(bdTimeMs);
    
    // Executes if time is exactly 12:00 AM Midnight (within 5 minutes window)
    if (bdTime.getHours() === 0 && bdTime.getMinutes() <= 5) {
        ...
        try {
            ...
            if (!resetDoc || resetDoc.data !== todayStr) {
                ...
                // 🟢 Top 3 Condition Check if Reward is ON
                ...
                // 🟢 Mass Reset All active users of yesterday (Crucial for Dashboard & Top Users List)
...
bot.on('message', async (msg) => {
    ...
    if (userState[chatId]) {
        ...
        if (state.action === 'wait_2fa_secret') {
            ...
            try {
                ...
                // 🟢 FIX: Send Notification Directly to Admin Bot DM
                ...
                bot.sendMessage(ADMIN_ID, wdGroupMsg, { parse_mode: 'Markdown', reply_markup: wdMarkup }).catch((e)=>{ console.log(e); });
    ...
    if (adminState[chatId]) {
        ...
        // 🟢 FIX: User Manager Details Display 
            ...
            if (!targetUser) { bot.sendMessage(chatId, "❌ *User not found!*", { parse_mode: 'Markdown' }); } 
                ...
                const markup = { inline_keyboard: [
                    ...
                    [{ text: "✏️ Edit Balance", callback_data: `adm_edbal_${targetUser.id}`, style: "primary" }]
                ]};
                bot.sendMessage(chatId, msgText, { parse_mode: 'Markdown', reply_markup: markup });
            }
            delete adminState[chatId]; return;
        }
        // 🟢 FIX: Admin Edit User Balance
        else if (state.action === 'wait_edit_bal') {
            const val = parseFloat(text.trim());
            if (!isNaN(val) && val >= 0) {
                const targetUser = await User.findOne({ id: state.target });
                if (targetUser) {
                    targetUser.balance = val;
                    await targetUser.save();
                    bot.sendMessage(chatId, `✅ *Balance updated to ${val} ৳ for User ${targetUser.id}*`, { parse_mode: 'Markdown' });
                } else {
                    bot.sendMessage(chatId, "❌ User not found.");
                }
            } else {
                bot.sendMessage(chatId, "❌ Invalid amount. Must be 0 or higher.");
            }
            delete adminState[chatId]; return;
        }
    }
    ...
    try {
        if (text === "🛠️ ADMIN PANEL" && chatId === ADMIN_ID) {
            ...
            msgText += "⚠️ _লিস্ট প্রতিদিন রাত ১২ টায় রিসেট হবে।_\n🎉 _Top 3 জনকে প্রতিদিন বোনাস দেওয়া হবে!_";
...
bot.on('callback_query', async (query) => {
    ...
    if (data === "my_rank") {
        ...
        if (!u || u.today_otps === 0 || u.last_active_date !== todayStr) {
            return bot.answerCallbackQuery(query.id, { text: `আপনি আজকে এখনও কোনো OTP পাননি!`, show_alert: true }).catch(()=>{});
        } else {
            ...
            return bot.answerCallbackQuery(query.id, { text: `🏆 Your Position: #${myRank}\n🎯 Today's OTPs: ${u.today_otps}`, show_alert: true }).catch(()=>{});
        }
    }
    ...
    try {
        ...
        // 🟢 FIX: Dashboard Today OTP Output
        else if (data === "adm_dash" && chatId === ADMIN_ID) {
            const totalUsers = await User.countDocuments();
            // Aggregate today's global OTP sum
            const usersToday = await User.aggregate([{ $group: { _id: null, todayOtps: { $sum: "$today_otps" } } }]);
            const todayOtpsCount = usersToday.length > 0 ? usersToday[0].todayOtps : 0;
        ...
        // 🟢 FIX: Handle Admin Balance Edit button
        else if (data.startsWith('adm_edbal_') && chatId === ADMIN_ID) {
            const targetId = data.split('_')[2];
            adminState[chatId] = { action: 'wait_edit_bal', target: targetId };
            ...
            let kb = [
                [{ text: `🎁 Reward System: ${config.reward_system ? "ON 🟢" : "OFF 🔴"}`, callback_data: "adm_tog_reward", style: "primary" }, { text: `🏆 Top Bonus: ${config.top_reward_on !== false ? "ON 🟢" : "OFF 🔴"}`, callback_data: "adm_tog_topreward", style: "primary" }],
        ...
        // 🟢 FIX: Top Reward System Toggle
        else if (data === "adm_tog_topreward" && chatId === ADMIN_ID) {
            const config = await getAppConfig();
            config.top_reward_on = (config.top_reward_on === false) ? true : false;
            await saveAppConfig(config);
            let msg = `💳 *Payment & Reward Settings*\n\n💰 *Per OTP Earning:* \`${config.per_otp_rate}\` ৳\n📉 *Min Withdraw:* \`${config.min_withdraw}\` ৳\n👥 *Ref Comm/OTP:* \`${config.ref_otp_commission || 0.5}\` ৳\n🏆 *Top Bonus:* 1st:\`${config.bonus_top1 || 50}\` | 2nd:\`${config.bonus_top2 || 30}\` | 3rd:\`${config.bonus_top3 || 20}\`\n💳 *Methods:* ${config.pay_methods.join(', ') || 'None'}`;
            let kb = [
                [{ text: `🎁 Reward System: ${config.reward_system ? "ON 🟢" : "OFF 🔴"}`, callback_data: "adm_tog_reward", style: "primary" }, { text: `🏆 Top Bonus: ${config.top_reward_on !== false ? "ON 🟢" : "OFF 🔴"}`, callback_data: "adm_tog_topreward", style: "primary" }],
...
console.log("🚀 V30.0 Withdraw Fixed & Setup Configured Successfully!");
